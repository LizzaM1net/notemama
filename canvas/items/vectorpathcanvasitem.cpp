...
QList<ColorVector2D> VectorPath::LineSegment::render(QVector2D startPoint) const
{
...
QList<ColorVector2D> VectorPath::QuadCurveSegment::render(QVector2D startPoint) const
{
    ...
    for (int i = 1; i < points-1; i++) {
        ...
        curve << ColorVector2D(point, (qSin(i)+1)/2, (qCos(i)+1)/2, 1) + (i%2==0 ? 1 : -1) * normal * width;
    }
    return curve;
}
...
QList<ColorVector2D> VectorPath::CubicCurveSegment::render(QVector2D startPoint) const
{
...
void VectorPathCanvasItem::synchronize(QRhiBuffer *buffer, QRhiResourceUpdateBatch *updateBatch)
{
    if (!changed)
        return;
    ...
    QList<ColorVector2D> vertexData = render();

    if (buffer->size() < vertexData.size()*sizeof(ColorVector2D)) {
        qDebug() << "resize" << vertexData.size();
        buffer->setSize(vertexData.size()*sizeof(ColorVector2D));
        buffer->create();
    }

    updateBatch->updateDynamicBuffer(buffer, 0, buffer->size(), vertexData.data());
}

QList<ColorVector2D> VectorPathCanvasItem::render()
{
    ...
    for (const VectorPath::Segment* segment : std::as_const(segments)) {
        ...
        normal /= QVector2D::dotProduct(tangent, segment->inTangent());

        points << ColorVector2D(point, Qt::black) - width*normal
               << ColorVector2D(point, Qt::black) + width*normal;

        points << segment->render(point);
